...
expectation $XSY^{T}$ where the entries are independently Poisson
...
opposed to the naive sampling approach, which is
$\mathcal{O}\left( n^{2} \right)$, where $n$ is the number of nodes in
the network. For additional details, see the
...
$\mathcal{O}\left( m + n + k^{2} \right)$ time) and random dot product
graphs (in $\mathcal{O}\left( n^{2}k \right)$ time).
